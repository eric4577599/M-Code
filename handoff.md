...
2. ~~假相機探針~~ **已完成(2026-08-04,`report20260804-1.md` §6c,規格 §5.4g)** ——
   ...
   **`roiSample` 實測 945×60,正好證實 §6b 的診斷。**
   順手補完 **DataMatrix 路徑實跑**(規格 §5.4h,`report20260804-1.md` §6d)——
   「已矯正 389×390;透視傾角 0.4°(單應矩陣實算)」。
   **三條分支(1D / QR v≥2 / DataMatrix)至此全部實跑過。**
...
- **寫探針要知道的:本 bundle 的 `MultiFormatWriter` 只接得到 QR 與 Aztec。**
  1D 一律回 `No encoder available`(要手刻編碼表);DataMatrix 也不接,但
  **`ZXing.DataMatrixWriter` 類別本身有匯出,直接 new 它就行**。下次不必再試錯。
- **`demo/*.html` 全程無自動化測試可覆蓋 —— 2026-08-04 這條界線真的咬人了。**
  ...
  座標換算與 canvas I/O 仍在 HTML 裡。**三條分支現已全部實跑過**(1D / QR v≥2 / DataMatrix)。
