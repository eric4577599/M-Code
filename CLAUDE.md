...
- 線上 docroot 是 **`~/m-code-site`**,不是 repo 本身。**上線一律跑腳本,不要手打三步**:
  ...
  **驗蓋章有沒有跑,不可以比 hash**(2026-08-22 稽核 A-13):
  ```bash
  curl -s https://m-code.ericchh.work/demo/sw.js | grep -c __STAMP__   # 必須是 0
  ```
  `sw.js` 是**唯一必須與 repo 不同**的檔案 —— 沒蓋章時它與 repo 完全相同,
  拿平常那套「比 hash,一致 = 同步好了」去驗,會把失敗判成成功。方向剛好相反。
